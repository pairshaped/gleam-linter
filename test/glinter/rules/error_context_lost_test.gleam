...
pub fn detects_replace_error_with_discard_test() {
  let results =
    test_helpers.lint_string(
      "import gleam/result
pub fn bad(r) { result.replace_error(r, \"oops\") }",
      error_context_lost.rule(),
    )
  // replace_error always discards — but our rule checks for fn(_) pattern,
  // and replace_error takes a value not a fn, so this should NOT trigger
